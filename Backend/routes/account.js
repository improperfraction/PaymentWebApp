...
router.post('/transfer', authmid, async (req, res) => {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            if (!payerDeducted) throw new Error('Something went wrong while deducting money from payer');
    ...
    try {
        ...
        res.status(200).json({
            ...
            transactions: transactions.map((tx) => {
                return {
                    Name: `${tx.counterParty.firstName[0].toUpperCase() + tx.counterParty.firstName.slice(1)} ${
                        tx.counterParty.lastName[0].toUpperCase() + tx.counterParty.lastName.slice(1)
                    }`,
                    amount: tx.amount,
                    status: tx.status,
